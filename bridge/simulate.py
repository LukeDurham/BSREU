...
class Simulator():
    ...
    def simulate_patient(self, patient):
        ...
        if not patient.get_prescriptions():
            ...
            ndc = 5555555555
            quantity = 5 #random.randint(1, 9)
            refills = 5 #random.randint(1, 9)
    ...
    def save_role_pool(self, path):
        ...
        df.to_csv(os.path.join('experiments', f'{path}.csv'))


if __name__ == '__main__':
    start = time.time()
    simulator = Simulator(2500, 10, 0.6, 0.3, 0.1)
    simulator.cycle()
    simulator.save_role_pool('2500')
    runtime = time.time() - start
    print(f'Total experiment time: {runtime}s')
